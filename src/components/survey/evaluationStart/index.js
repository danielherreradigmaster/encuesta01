import './styles.scss';

const EvaluationStart = () => {
  return (
    <div className='evaluation-start'>
      <div className='evaluation-start__content'>
        <h1 className='evaluation-start__title'>Evaluación del nivel de madurez</h1>
        <p className='evaluation-start__description'>A continuación te presentamos 7 preguntas para que puedas conocer el nivel de madurez de tu Organización según la Metodología de RIMS.</p>
        <p className='evaluation-start__required'>Todas las preguntas son obligatorias *</p>
      </div>
    </div>
  )
};

export default EvaluationStart;